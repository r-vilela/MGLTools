...
class GridParameterFileMaker:
    ...
    def read_reference(self, reference_filename):
        if self.verbose:
            print "reading ", reference_filename
    ...
    def set_ligand(self, ligand_filename):
        ...
        if self.verbose:
            print "set ligand_stem to", self.ligand_stem
        self.ligand = Read(ligand_filename)[0]
        # IS THIS USEFUL???
        ...
        if self.verbose:
            print "set types to ", self.gpo['types']['value']
        # gridcenter
    ...
    def getSideLengths(self, mol):
        ...
        mol.npts = map(int, map(ceil, sideLengths /
                       (self.gpo['spacing']['value'])))
...
class GridParameter4FileMaker:
        ...
        for ix, val in enumerate(self.gpo['npts']['value']):
            ...
            if hasattr(self.ligand, 'npts'):
                ...
                if npts[ix] > val:
                    self.gpo['npts']['value'][ix] = npts[ix]
        if self.verbose:
            print "set npts to ", self.gpo['npts']['value']
    ...
    def set_receptor(self, receptor_filename, gpf_filename=None):
        ...
        if self.receptor == None:
            ...
            return
        if self.verbose:
            print "set_receptor filename to ", receptor_filename
